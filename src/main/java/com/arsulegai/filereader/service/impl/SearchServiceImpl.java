...
public class SearchServiceImpl implements SearchService {
  ...
  public List<String> search(String query) {
    ...
    Query nativeSearchQuery = new NativeSearchQueryBuilder()
            ...
            .build();
    SearchHits<AppFileUnwrapped> foundAppFiles = elasticsearchTemplate.search(nativeSearchQuery, AppFileUnwrapped.class);
    // find the file path with some information of the file
    List<String> files = foundAppFiles.stream().map(searchHit -> searchHit.getContent().getAppFile().getPath()).collect(Collectors.toList());
    return files;
  }
}
